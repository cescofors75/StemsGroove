...
export async function warmupClientDemixer(options = {}) {
  ...
  if (shouldTryWebGpu) {
    try {
      session = await createOrtSession(ort, manifest.modelUrl, ["webgpu", "wasm"]);
    } catch (error) {
      ...
      configureWasmRuntime(ort);
      session = await createOrtSession(ort, manifest.modelUrl, ["wasm"]);
      provider = "wasm";
    }
  } else {
    session = await createOrtSession(ort, manifest.modelUrl, ["wasm"]);
  ...
  console.log(
    `[client-demixer] engine=${providerLabel} · crossOriginIsolated=${isolated} · wasmThreads=${threads} · dspWasm=${!!dspBackend}`,
  );
